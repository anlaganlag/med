...
|----------------|-------------------------|------------|-------|------------|---------------------------|
| **基础云服务** |                         |            |       |            |                          |
| 云服务器(ECS)  | 4核8G通用型             | 0.8万/年   | 10台  | 8.0        | 阿里云包年包月           |
| 对象存储(OSS)  | 10TB标准存储            | 1.2万/年   | 1     | 1.2        | 医疗影像数据存储         |
| **AI服务**     |                         |            |       |            |                          |
| 基础语言模型调用 | Qwen3 API              | 0.06元/1K tokens | 200万tokens | 0.8    | 主要模型调用         |
| 医疗专用模型调用 | ChatGLM-Med API        | 0.08元/1K tokens | 50万tokens | 0.2     | 医疗领域优化                          |
| **模型训练服务** |                         |            |       |            |                          |
| 定制化医疗推荐建模 | 阿里云                | 2.0万/次   | 5次   | 10.0       | 模型定制化训练           |
| 模型微调(优化阶段) | 阿里云                | 1.5万/次   | 12次  | 18.0       | 模型定制化训练           |
| 估算含优化阶段（**合计**） |                 |   |   | **38.2**       |            



> 说明：
> 1. 成本测算周期为完整项目周期（2年），云服务类按年度报价测算
> 2. 备用金包含系统升级、紧急扩容等非预期支出
> 3. 实际支出将根据项目推进情况动态调整，允许±15%浮动
